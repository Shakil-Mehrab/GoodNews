@extends('layouts.master')
 @section('leftside')
				...
				<div class="featured-posts gn-animation" data-animation="fadeInUp" data-animation-delay="0" data-animation-offset="75%">
						<div class="content">
						@forelse($politics_news_top as $politic)
						<?php
						Session::put('politics_new_top_id',$politic->id);
					   ?>
						<article class="post">
								<div class="thumb">
								<a href="{{URL::to($politic->image)}}">
								<img  src="{{URL::to($politic->image)}}" class="top" alt="img">
								...
								<span style="text-align:center"><h3><a href="{{route('new.single',$politic->id)}}">{{$politic->heading}}</a></h3></span>
								<span style="text-align:justify"><p class="excerpt-entry">@php echo str_limit($politic->description,300) @endphp<a  href="{{route('new.single',$politic->id)}}">Read more</a></p></span>
								
								<div class="post-meta">
									<span class="author">By <a href="#">{{$politic->user->name}}</a></span>
								</div>
				...
				<div class="highlights-posts gn-animation" data-animation="fadeInUp" data-animation-delay="0" data-animation-offset="75%">
						...
						<div class="section-title">
							<h4><a href="{{route('national.index')}}">Politics</a></h4>
						</div>
					
						@forelse($politics_news_mid as $politic)
						@if($politic->id%2==0)
						<?php
						Session::put('politics_new_mid_id',$politic->id);
					   ?>
						<article class="post">
							<div class="thumb">
							<a href="{{URL::to($politic->image)}}">
									<img  src="{{URL::to($politic->image)}}" class="national" alt="img">
									</a>
							</div>
							<div class="cat">
								<a href="#">{{$politic->media}}</a>
							</div>
							<h3><a href="{{route('new.single',$politic->id)}}">{{$politic->heading}}</a></h3>
							<span style="text-align:justify"><p class="excerpt-entry">@php echo str_limit($politic->description,250) @endphp<a href="{{route('new.single',$politic->id)}}">Read more</a></p></span>
							...
							<div class="content-wrapper">
										...
										<div class="card-body">
											...
											<div class="col-12">
												...
												<tbody>
													@forelse($politics_news as $national_new)
													<tr>
														<td>
														<article class="post">
															<div class="thumb">
