...
<span class="views">{{$new->views}}</span><span class="comment" onclick="toggleCommnet('{{$new->id}}')"><a>{{$new->comments()->count()}}</a></span>
        <div class="comment-form-{{$new->id}} hidden">
        <form  action="{{route('comment.store',$new->id)}}" method="POST"  class="inline-it">
        {{csrf_field()}}  
